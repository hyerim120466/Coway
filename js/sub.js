    ...
    const showItem = (data) => {
        ...
        compareBtn.forEach((btn, idx) => {
            btn.addEventListener('click', (e) => {
                ...
                // 비교 제품 4개 이상일 때
                else if (pressBtnCnt > 4) {
                    showPopup('최대 4개 제품까지 비교할 수 있습니다.');
                }
                // 비교담기 이미 클릭 시, 재클릭
                else showPopup('비교담기 해제는 아래 비교하기 탭에서 가능합니다.');
            });
        });
...
const introduce = () => {
    ...
    const $storeImg1 = get('.introduce .store-store .store-slide-box .to-store li .img1');
    ...
    let cnt = 1;
    const make = () => {
        $storeName.textContent = stn[cnt].name;
        $storeAddress.textContent = stn[cnt].address;
        $storeImg2.setAttribute('src', stn[cnt].img);
        $storeImg2.setAttribute('alt', stn[cnt].alt);

        const prevIndex = cnt === 0 ? stn.length - 1 : cnt - 1;
        const nextIndex = cnt === stn.length - 1 ? 0 : cnt + 1;

        $storeImg1.setAttribute('src', stn[prevIndex].img);
        $storeImg1.setAttribute('alt', stn[prevIndex].alt);
        $storeImg3.setAttribute('src', stn[nextIndex].img);
        $storeImg3.setAttribute('alt', stn[nextIndex].alt);
    };
    make();
    //  click시 이미지 변경
    $introNext.addEventListener('click', () => {
        cnt++;
        if (cnt >= stn.length) cnt = 0;
        make();
    });

    $introPrev.addEventListener('click', () => {
        cnt--;
        if (cnt < 0) cnt = stn.length - 1;
        make();
    });
};
// 기타판매처
...
const reservation = () => {
    ...
    function updateStoreInfo(store) {
        if ($getName && $getAdd && $getTime && $getCloe && $getPark && $getTel) {
            ...
            console.error('One or more target elements are not found.');
        }
    }
...
const productService = () => {
    ...
    const slide2 = () => {
        const $scrolled = get('.pro_service_sec3 .gray_box .scroll');
        let isDown = false;
        let startX;
        let scrollLeft;

        $scrolled.addEventListener('mousedown', (e) => {
            isDown = true;
            $scrolled.classList.add('active');
            startX = e.pageX - $scrolled.offsetLeft;
            scrollLeft = $scrolled.scrollLeft;
        });

        $scrolled.addEventListener('mouseleave', () => {
            isDown = false;
            $scrolled.classList.remove('active');
        });

        $scrolled.addEventListener('mouseup', () => {
            isDown = false;
            $scrolled.classList.remove('active');
        });

        $scrolled.addEventListener('mousemove', (e) => {
            if (!isDown) return; // 마우스가 클릭되지 않았으면 함수 종료
            e.preventDefault();
            const x = e.pageX - $scrolled.offsetLeft;
            const walk = (x - startX) * 2; // 스크롤 속도 조정
            $scrolled.scrollLeft = scrollLeft - walk;
        });
    };
    tap1();
    slide1();
...
const iocareService = () => {
    const iocEvent1 = () => {
        ...
        // 스크롤 이벤트에 대한 리스너 추가
        window.addEventListener('scroll', iocEventHandler);

        // 페이지 로드 시 처음으로 이벤트 핸들러 호출
        iocEventHandler();
    };
    const iocEvent2 = () => {
        const iocEventHandler2 = () => {
            // 모든 대상 요소 가져오기
            ...
            textElements.forEach((textElement, index) => {
                ...
                // 요소가 뷰포트 내에 있고 아직 애니메이션이 적용되지 않은 경우
                if (rect.top <= 0 && rect.bottom >= 0 && textElement.style.opacity !== '1') {
                    setTimeout(() => {
        ...
        // 스크롤 이벤트에 대한 리스너 추가
        window.addEventListener('scroll', iocEventHandler2);
...
const filterService = () => {
    ...
    const filterAni1 = () => {
        const iocEventHandler2 = () => {
            // 모든 대상 요소 가져오기
            const textElements = getAll('.filter_service_sec2 ul li');
            const windowHeight = window.innerHeight;
            ...
            textElements.forEach((textElement, index) => {
                ...
                // 요소가 뷰포트 내에 있고 아직 애니메이션이 적용되지 않은 경우
                if (rect.top < windowHeight - 200 && textElement.style.opacity !== '1') {
                    setTimeout(() => {
                        textElement.style.animation = ` leftToRight ease 1.5s ${delay}s forwards`;
                        textElement.style.opacity = 1;
                    }, 200);
...
const joinPage3 = () => {
    ...
    $infoName.addEventListener('input', (e) => {
        ...
        // 숫자와 특수문자를 제외한 문자(한글, 영어)를 찾는 정규 표현식
        // 이거 다시 손보기
        const invalidPattern1 = /[ㄱ-ㅎ가-힣a-zA-Z]/;
...
const myPage = () => {
    ...
    $goingPros[0].addEventListener('click', (e) => {
        location.href = '/page/products/berexelc.html';
        // localstorage 사용 (보완)
    ...
    $goingPros[1].addEventListener('click', (e) => {
        location.href = '/page/products/lifeelc.html';
        // localstorage 사용 (보완)
        // if (location.pathname.split('/').pop() === 'berexelc.html') localStorage.setItem('productDetail', JSON.stringify(objectData[5]));
    });
